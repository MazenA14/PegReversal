...
isGoal:: Board-> Bool
isGoal [] = True
isGoal (Peg _ color:t) | color == W = isGoal t
					   | otherwise = False
 
 
showPossibleNextStates:: Board-> [State]
showPossibleNextStates currentBoard | not (statesHelper currentBoard currentBoard []==[])= statesHelper currentBoard currentBoard []
									| otherwise = error "No Possible States Exist."
...
checkColorW (Peg position color:t) pos | checkPos pos (Peg position color:t)==False=False
									   |(position==pos && checkPos pos (Peg position color:t) ) = color==W
...
statesHelper [] _ acc=acc
statesHelper (Peg index color:t) currentBoard acc | isValidMove (M index) currentBoard = statesHelper t currentBoard (acc++ [S (M index) (changeColor index currentBoard []) ])
									              | otherwise = statesHelper t currentBoard acc